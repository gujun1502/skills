...
- **Lead with forward-looking (Layer 4) High risks** — schedule slippage, scope/workload creep, future contingencies, and hidden traps — because these surface only during performance and are the easiest for the business team to overlook. Where exposure can be quantified (e.g., uncapped revision rounds, uncapped liability, fixed price over a multi-year term), state it concretely.
